...
void parse_image()
{
    ...
    {
        //set file position on staer where file signature(ID) is->bmp
        offset = 0; 
        ...
        //Get offset from pixel array start 
        offset = 10;  
      
        fseek(image_file, offset, SEEK_SET); 
         // Bitmap data offset 
        fread(&bmpdataoff, 4, 1, image_file);
        //Read height and height width=18 offset,height=22 offset each 4 bytes
        fread(&width, 4, 1, image_file); 
      
        fread(&height, 4, 1, image_file);  
        
        //nr of bits per pixel
    ...
    //find nr of non-zero occurence,also find nr of leaf nodes in Image
    int nodes = 0;
    for(i=0;i<256;i++)
    {
        if(hist[i] != 0)
   ...
   struct pixfreq  
    ...
    for(i=0;i<256;i++)
    {
        if(hist[i] != 0)
        {
            ...
            //location of the node in pix_freq array
            ...
            //initializing the code words as EOF
            pix_freq[j].code[0] = '\0';
            j++;
        }
    }
    ...
    while (n < nodes - 1)  
    {
        //adding thte lowest two probabilities
        ...
        //arrloc points to the locations of child node in pix_freq array
